...
for i in range(500):
    ...
    if base == 'Z':
        if bit == 1:
            qc.x(0)  # For base Z, flip to |1> if bit is 1
    else:  # base == 'X'
        # Instead of using the built-in H, we replace it by: Rz(pi) followed by Ry(pi/2)
        if bit == 0:
            qc.rz(pi, 0)
            qc.ry(pi/2, 0)  # Produces state equivalent to |+>
        else:
            qc.x(0)        # Flip the state to |1>
            qc.rz(pi, 0)
            qc.ry(pi/2, 0)  # Produces state equivalent to |->
...
bob_bases = [random.choice(['Z', 'X']) for _ in range(500)]
print("bob bases:")
print("".join(map(str, bob_bases)))

# Step 4: Bob measures Alice's qubits
bob_bits = []
backend = Aer.get_backend('qasm_simulator')  # QASM simulator for measurements
...
for i in range(500):
    ...
    if base == 'Z':
        qc.measure(0, 0)  # Measure in the Z basis
    else:  # For X basis measurement, replace H with our sequence:
        qc.rz(pi, 0)
        qc.ry(pi/2, 0)
        qc.measure(0, 0)
...
# Step 6: Generating secret keys (keeping only positions where bases match)
