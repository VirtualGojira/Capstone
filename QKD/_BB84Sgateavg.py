...
for _ in range(num_runs):
    ...
    for i in range(500):
        ...
        if base == 'Z':
            ...
            if bit == 0:
                qc.s(0)  # Apply S gate to put the qubit in |+> state if bit is 0
            else:
                qc.x(0)
                qc.s(0)  # Apply X and S gates to transform to |-> state if bit is 1
    ...
    for i in range(500):
        ...
        if base == 'Z':
            ...
            qc.s(0)  # Apply H gate to change from Z basis to X basis
            qc.measure(0, 0)
