...
def create_combined_bb84_circuit(bit, alice_base, bob_base):
    ...
    - Alice's preparation using the alternative Hadamard:
      H ∝ Rz(π/2) → Rx(π) → Rz(π/2)
    ...
    if alice_base == 'Z':
        if bit == 1:
            qc.x(0)  # For Z basis, flip to |1> if bit is 1
    else:  # Alice in X basis
        if bit == 0:
            # Prepare |+> using: Rz(pi/2), then Rx(pi), then Rz(pi/2)
            qc.rz(pi/2, 0)
            qc.rx(pi, 0)
            qc.rz(pi/2, 0)
        else:
            qc.x(0)  # Flip to |1>
            # Prepare |-> state with the same sequence
            qc.rz(pi/2, 0)
            qc.rx(pi, 0)
            qc.rz(pi/2, 0)
    
    # Bob's measurement: if Bob measures in the X basis, apply the same sequence
    if bob_base == 'X':
        qc.rz(pi/2, 0)
        qc.rx(pi, 0)
        qc.rz(pi/2, 0)
