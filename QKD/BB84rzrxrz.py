...
for i in range(500):
    ...
    if base == 'Z':
        ...
        if bit == 1:
            qc.x(0)  # For bit 1, first flip qubit to |1>
        # Replace the standard Hadamard gate with: Rz(pi/2) then Rx(pi) then Rz(pi/2)
        qc.rz(pi/2, 0)
        qc.rx(pi, 0)
        qc.rz(pi/2, 0)
...
for i in range(500):
    ...
    else:  # For X basis measurement, apply the custom Hadamard decomposition:
        qc.rz(pi/2, 0)
        qc.rx(pi, 0)
        qc.rz(pi/2, 0)
        qc.measure(0, 0)
...
# Step 6: Generating secret keys (only keep bits where Alice and Bob used the same basis)
