...
def create_combined_bb84_circuit(bit, alice_base, bob_base):
    ...
    - Alice's preparation (T gates instead of H)
    - Bob's measurement (T gates instead of H)
    """
    ...
    else:  # Alice in X basis (Modified: Use T gate instead of H)
        if bit == 0:
            qc.t(0)  # Apply T gate (π/4 phase shift)
        else:
            qc.x(0)
            qc.t(0)  # Apply T gate
    ...
    # Bob's measurement
    if bob_base == 'X':
        qc.t(0)  # Apply T gate before measurement (instead of H)
...
# Generate grids
visualize_grid("combined_circuit", "BB84 Combined Circuit Grid with T Gate")
visualize_grid("bloch", "BB84 Bloch Sphere Grid with T Gate")
