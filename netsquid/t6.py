...
from netsquid.components import QuantumChannel, ClassicalChannel, Message, DepolarNoiseModel
...
def create_network():
    ...
    q_channel.models["quantum_noise_model"] = DepolarNoiseModel(depolar_rate, time_independent=True)
...
def run_bb84_kem(num_bits):
    ...
    # Loss Analysis
    sent_bits = len(alice_protocol.bits)
    received_bits = len(bob_protocol.measurements)
    lost_bits = sent_bits - received_bits
    loss_rate = lost_bits / sent_bits if sent_bits > 0 else 0
    
    # QBER Calculation
    qber = 0
    if received_bits > 0:
        # Compare received bits with Alice's original bits
        errors = sum(a != b for a, b in zip(alice_protocol.bits[:received_bits], bob_protocol.measurements))
        qber = errors / received_bits
    
    print(f"\nLoss Rate: {loss_rate:.2%} ({lost_bits}/{sent_bits})")
    print(f"QBER: {qber:.2%} ({errors}/{received_bits})")
...
def avg_100_runs(n_bits):
    ...
    for _ in range(trials):
        success, loss_rate, qber = run_bb84_kem(n_bits)
        total_loss += loss_rate
        total_qber += qber
        if success:
            success_count += 1
    avg_loss = total_loss / trials
    avg_qber = total_qber / trials
    success_rate = success_count / trials
    print(f"\nAverage Loss Rate ({trials} runs): {avg_loss:.2%}")
    print(f"Average QBER ({trials} runs): {avg_qber:.2%}")
    print(f"Key Agreement Success Rate: {success_rate:.2%}")
