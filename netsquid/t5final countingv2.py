...
def run_bb84_kem(num_bits):
    ...
    alice_protocol = AliceProtocol(alice, num_bits, bob_kem)
...
def run_bb84_kem_with_log(num_bits):
    ns.sim_reset()
    network = create_network()
    ...
    # Run simulation and capture the simulation summary log
    sim_log = ns.sim_run(end_time=ENDTIME)
    success = (alice_protocol.bits.tolist() == bob_protocol.measurements)
    return success, sim_log

def parse_quantum_ops(sim_log):
    """
    Extracts counts for quantum operations from the simulation log.
    The line starting with "Frequent quantum operations:" is assumed to be present,
    but the order of the key-value pairs may vary.
    
    Returns a dictionary with keys as operation names (e.g., "H", "X", "MEASURE")
    and values as their respective counts.
    """
    # Convert sim_log to a string in case it's not already
    sim_log_str = str(sim_log)
    lines = sim_log_str.splitlines()
    op_line = None
    for line in lines:
        if "Frequent quantum operations" in line:
            op_line = line
            break
    if not op_line:
        return {}
    
    # Use regex to extract key=value pairs; order doesn't matter.
    matches = re.findall(r'(\w+)\s*=\s*(\d+)', op_line)
    op_counts = {key: int(val) for key, val in matches}
    return op_counts


def avg_100_runs_collect_log(n_bits, runs=100):
    all_counts = {"H": [], "X": [], "MEASURE": []}
    successes = 0
    for i in range(runs):
        success, sim_log = run_bb84_kem_with_log(n_bits)
        if success:
            successes += 1
        op_counts = parse_quantum_ops(sim_log)
        # Append counts for each operation if available, otherwise use 0.
        all_counts["H"].append(op_counts.get("H", 0))
        all_counts["X"].append(op_counts.get("X", 0))
        all_counts["MEASURE"].append(op_counts.get("MEASURE", 0))
    return successes, all_counts

def plot_bar_box(all_counts):
    op_names = list(all_counts.keys())
    avg_counts = [np.mean(all_counts[op]) for op in op_names]
    
    # Bar plot: average counts
    plt.figure(figsize=(8, 6))
    plt.bar(op_names, avg_counts, color=['blue', 'green', 'red'])
    plt.xlabel("Quantum Operations")
    plt.ylabel("Average Count (over runs)")
    plt.title("Average Frequency of Quantum Operations (from simulation logs)")
    plt.show()
    
    # Box plot: distribution of counts
    plt.figure(figsize=(8, 6))
    data = [all_counts[op] for op in op_names]
    plt.boxplot(data, labels=op_names)
    plt.xlabel("Quantum Operations")
    plt.ylabel("Operation Count")
    plt.title("Distribution of Quantum Operation Counts (from simulation logs)")
    plt.show()

#run_bb84_kem(NO_OF_BITS)
#manual_survey(NO_OF_BITS)
#avg_100_runs(NO_OF_BITS)

df = pd.read_csv("BO_algorithm_results.csv")
results = {}

runs_per_config = 100

for idx, row in df.iterrows():
    bits_val = int(row['Bits'])
    # Update global simulation parameters with values from the CSV row.
    NO_OF_BITS = bits_val
    DISTANCE = float(row['Best Distance'])
    TIMEOUT = float(row['Best Timeout'])
    MAX_RETRIES = int(row['Best Max Retries'])
    
    # Lists to store counts for each quantum operation.
    counts_H = []
    counts_X = []
    counts_MEASURE = []
    
    for i in range(runs_per_config):
        success, sim_log = run_bb84_kem_with_log(NO_OF_BITS)
        op_counts = parse_quantum_ops(sim_log)
        counts_H.append(op_counts.get("H", 0))
        counts_X.append(op_counts.get("X", 0))
        counts_MEASURE.append(op_counts.get("MEASURE", 0))
    
    results[bits_val] = {"H": counts_H, "X": counts_X, "MEASURE": counts_MEASURE}

bits_values = sorted(results.keys())
avg_H = [np.mean(results[b]["H"]) for b in bits_values]
avg_X = [np.mean(results[b]["X"]) for b in bits_values]
avg_MEASURE = [np.mean(results[b]["MEASURE"]) for b in bits_values]

x = np.arange(len(bits_values))  # positions for each configuration group
width = 0.25  # width for each bar

plt.figure(figsize=(12, 6))
plt.bar(x - width, avg_MEASURE, width, label="MEASURE", color='orange')
plt.bar(x, avg_H, width, label="H", color='blue')
plt.bar(x + width, avg_X, width, label="X", color='green')
plt.xticks(x, [str(b) for b in bits_values])
plt.xlabel("Bits")
plt.ylabel("Average Operation Count")
plt.title("Average Quantum Operation Counts per Configuration")
plt.legend()
plt.tight_layout()
...
# For MEASURE
data_measure = [results[b]["MEASURE"] for b in bits_values]
axes[0].boxplot(data_measure, labels=[str(b) for b in bits_values])
axes[0].set_title("MEASURE")
axes[0].set_xlabel("Bits")
axes[0].set_ylabel("Operation Count")

# For H
data_H = [results[b]["H"] for b in bits_values]
axes[1].boxplot(data_H, labels=[str(b) for b in bits_values])
axes[1].set_title("H")
axes[1].set_xlabel("Bits")

# For X
data_X = [results[b]["X"] for b in bits_values]
axes[2].boxplot(data_X, labels=[str(b) for b in bits_values])
axes[2].set_title("X")
axes[2].set_xlabel("Bits")

fig.suptitle("Distribution of Quantum Operation Counts per Configuration")
plt.tight_layout()
